...
def jump_euler(no_jump_fn, Dfun, jump_fn, jump_rate_fn, X0, ts, Us,
    ...
    This method integrates things in a naive way. Should update to do the way
    `integrate_tr_dec_no_jump` does it in integrators like the hierarchy jump
    integrator.
...
def milstein(drift, diffusion, b_dx_b, X0, ts, Us):
    ...
    for t, dt, dW in zip(ts[:-1], dts, dWs):
        X = np.vstack((X, X[-1] + drift(t, X[-1])*dt + diffusion(t, X[-1])*dW +
                       b_dx_b(t, X[-1])*(dW**2 - dt)/2))
