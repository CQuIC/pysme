...
def bi_dx_bj(G_i, G_j, G_j_G_i, k_T_i, k_T_j, k_T_j_G_i, rho):
    r"""A term in Taylor integration methods for heterodyne detection.

    Possibility to have 2 different :math:`b` corresponding to the 2 different processes
    in our heterodyne implementation.

    Function to return the :math:`\left(\vec{b}^i(\vec{\rho})\cdot
    \vec{\nabla}_{\vec{\rho}}\right)\vec{b}^j(\vec{\rho})` term for Milstein
    integration.

    Parameters
    ----------
    G_i: numpy.array
        :math:`G^i`, for the first index.
    G_j: numpy.array
        :math:`G^j`, for the second index.
    G_j_G_i : numpy.array
        :math:`G^j G^i`, like :math:`G^2` but for the first :math:`G` being
        for the process 'i' and the second for the process 'j'.
    k_T_i: numpy.array
        :math:`\vec{k}^T^i`.
    k_T_j: numpy.array
        :math:`\vec{k}^T^j`.
    k_T_j_G_i: numpy.array
        :math:`\vec{k}^T^jG^i`.
    rho: numpy.array
        :math:`\rho`.
    ...
    -------
    numpy.array
        :math:`\left(\vec{b}^i(\vec{\rho})\cdot
        \vec{\nabla}_{\vec{\rho}}\right)\vec{b}^j(\vec{\rho})`.
...
def multi_int_ij(dW_i, dW_j, p, dt):
    r"""Approximation of the multiple Ito integrals for non-commuting noise. Not working.
    Using Stratonovich-Taylor expansion method of Kloeden & Platen;
...
class HomodyneLindbladIntegrator(UncondLindbladIntegrator):
    r"""Integrator for an unconditional heterodyne Lindblad master equation.
...
class EulerHeterodyneIntegrator(Strong_0_5_HeterodyneLindbladIntegrator):
    ...
    def integrate(self, rho_0, times, U1s_1=None, U1s_2=None, U2s_1=None, U2s_2=None):
        ...
        times: numpy.array
            A sequence of time points for which to solve for rho
        U1s_1: numpy.array(len(times) - 1)
            Samples from a standard-normal distribution used to construct
            Wiener increments :math:`\Delta W` for each time interval. Multiple
            rows may be included for independent trajectories.
            For the first process, i.e. the first quadrature.
        U1s_2: numpy.array(len(times) - 1)
            Same as ``U1s_1`` but for the second quadrature.
        U2s_1: numpy.array(len(times) - 1)
            Unused, included to make the argument list uniform with
            higher-order integrators. For the first quadrature.
        U2s_2: numpy.array(len(times) - 1)
            Unused, included to make the argument list uniform with
            higher-order integrators. For the second quadrature.
    ...
    def integrate_measurements(self, rho_0, times, dMs_1, dMs_2):
        ...
        times: numpy.array
            A sequence of time points for which to solve for rho
        dMs_1: numpy.array(len(times) - 1)
            Incremental measurement outcomes used to drive the SDE for the
            first quadrature.
        dMs_2: numpy.array(len(times) - 1)
            Same as ``dMs_1``, but for the second measured quadrature.
...
class MilsteinHeterodyneIntegrator(Strong_1_0_HeterodyneLindbladIntegrator):
    ...
    def bi_dx_bj_fn(self, rho, t, i, j):
        '''Computes the last term of the milstein integration scheme.
